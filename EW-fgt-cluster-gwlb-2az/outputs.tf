...
output "fgt_mgmt" {
  value = {
    fgt1_mgmt   = "https://${one(module.fgt_nis.fgt_ni_list["az1.fgt1"]["public_eips"])}:${local.admin_port}"
    fgt2_mgmt   = "https://${one(module.fgt_nis.fgt_ni_list["az2.fgt1"]["public_eips"])}:${local.admin_port}"
    fgt_user    = "admin"
